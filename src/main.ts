...
function renderResultScreen(): void { clearGameTimer(); clearAnswerFeedbackTimer(); const game = controller.getState().game; const won = game.phase === "won"; const headline = won ? "Geschafft!" : "Die Zeit ist vorbei"; const message = won ? "Du hast alle Aufgaben vor Ablauf der zehn Minuten gelöst." : "Dein Ergebnis steht fest."; const highscoreMessage = resultEvaluation?.isNewPersonalBest ? "🏆 Neuer persönlicher Highscore!" : `Persönlicher Rekord: ${resultEvaluation?.personalBest.score ?? loadPersonalHighscore(student.studentId)?.score ?? 0}`; const greetingName = getGreetingName(student.name); const earnedStars = [resultEvaluation?.personalBest.stern1, resultEvaluation?.personalBest.stern3]; app.innerHTML = `<main class="app-shell result-screen"><section class="result-card" aria-labelledby="result-title"><div class="result-icon" aria-hidden="true">${won ? "✓" : "★"}</div><p class="eyebrow">Einmaleins</p><p class="result-greeting">Gut gespielt, ${escapeHtml(greetingName)}!</p><h1 id="result-title">${headline}</h1><p class="result-copy">${message}</p><div class="result-stars" aria-label="Freigeschaltete Sterne">${earnedStars[0] ? `<img src="/Stern1.png" alt="Stern 1" />` : ""}${earnedStars[1] ? `<img src="/Stern3.png" alt="Stern 3" />` : ""}</div><div class="result-stats" aria-label="Spielergebnis"><div class="result-stat"><span class="stat-label">Punkte</span><strong>${game.score}</strong></div><div class="result-stat"><span class="stat-label">Aufgaben</span><strong>${game.completedTasks} / ${TOTAL_TASKS}</strong></div><div class="result-stat"><span class="stat-label">Modus</span><strong>${practiceMode === "highscore" ? "Mit Highscore" : "Ohne Highscore"}</strong></div></div><div class="result-highscore" aria-live="polite">${highscoreMessage}</div><button type="button" class="result-button" id="again">Noch eine Runde</button></section></main>`; getRequiredElement<HTMLButtonElement>("#again").addEventListener("click", () => { clearWrongAnswerTimer(); startGame(); }); }
