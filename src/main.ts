    ...
    <main class="app-shell start-screen">
      <section class="welcome-card" aria-labelledby="welcome-title">
        ...
        ${showManualProfile ? `
          <details class="profile-panel">
            ...
            <form id="profile-form" class="profile-form">
              <label><span>Name</span><input id="student-name" name="name" type="text" maxlength="30" autocomplete="name" value="${escapeHtml(student.name)}" required /></label>
              <label><span>Klasse <small>(optional)</small></span><input id="student-class" name="className" type="text" maxlength="20" autocomplete="off" value="${escapeHtml(student.className ?? "")}" /></label>
        ...
        <div class="mode-actions" aria-label="Übungsmodus wählen">
          <button type="button" class="mode-card mode-card-primary" data-mode="highscore"><span class="mode-title">Üben mit Highscore</span><span class="mode-copy">Dein Ergebnis kann deinen persönlichen Rekord verbessern.</span></button>
          <button type="button" class="mode-card" data-mode="free"><span class="mode-title">Üben ohne Highscore</span><span class="mode-copy">Spiele dieselbe Runde ohne persönlichen Rekord zu verändern.</span></button>
        </div>
      </section>
    </main>`;
...
function renderGameScreen(): void {
  app.innerHTML = `
    <main class="app-shell game-screen">
      ...
      <section class="keypad" aria-label="Zahlenfeld">${[1,2,3,4,5,6,7,8,9].map((digit) => `<button type="button" class="keypad-key" data-digit="${digit}">${digit}</button>`).join("")}<button type="button" class="keypad-key keypad-zero" data-digit="0">0</button></section>
    </main>`;
...
function scheduleNextTaskAfterWrongAnswer(): void {
  ...
  wrongAnswerTimer = window.setTimeout(() => { wrongAnswerTimer = null; controller.advanceAfterWrongAnswer(); renderGameState(); }, 1000);
}

function clearGameTimer(): void {
  if (gameTimer !== null) { window.clearInterval(gameTimer); gameTimer = null; }
}
