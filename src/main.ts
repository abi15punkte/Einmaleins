...
async function submitSchoolHighscore(): Promise<void> { const status = document.querySelector<HTMLElement>("#school-status"); if (!status || !leaderboardClient) return; status.textContent = "Eintrag wird gespeichert …"; const game = controller.getState().game; const record = { studentId: student.studentId, name: student.name, className: student.className, score: game.score, achievedAt: new Date().toISOString() }; try { await leaderboardClient.submit(record); status.textContent = "Erfolgreich eingetragen."; } catch (error) { status.textContent = error instanceof Error ? error.message : "Eintrag konnte nicht gespeichert werden."; } }
