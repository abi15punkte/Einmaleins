...
function renderGameState(): void {
  ...
  round.textContent = `Durchlauf ${game.round}`; roundCaption.textContent = `Durchlauf ${game.round}`; taskNumberCaption.textContent = `Aufgabe ${Math.min(game.completedTasks + 1, TOTAL_TASKS)} von ${TOTAL_TASKS}`; score.textContent = String(game.score); time.textContent = formatTime(game.elapsedMs); progress.style.width = `${Math.min((game.completedTasks / TOTAL_TASKS) * 100, 100)}%`;
