...
async function checkForLatestBuild(): Promise<boolean> {
  ...
  buildCheckPromise = (async () => {
    try {
      ...
      if (!payload || typeof payload !== "object" || !("buildId" in payload) || typeof payload.buildId !== "string") {
        return true;
      }
      ...
      if (attemptedReload === `${CURRENT_BUILD_ID}->${latestBuildId}`) return true;
