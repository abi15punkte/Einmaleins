...
function renderResultScreen(): void { clearGameTimer(); clearAnswerFeedbackTimer(); const game = controller.getState().game; const won = game.phase === "won"; const headline = won ? "Geschafft!" : "Dein Ergebnis steht fest:"; const message = won ? "Du hast alle Aufgaben vor Ablauf der zehn Minuten gelöst." : ""; const highscoreMessage = resultEvaluation?.isNewPersonalBest ? "🏆 Neuer persönlicher Highscore!" : `Persönlicher Rekord: ${resultEvaluation?.personalBest.score ?? loadPersonalHighscore(student.studentId)?.score ?? 0}`; const greetingName = getGreetingName(student.name); const earnedStars = [resultEvaluation?.personalBest.stern1 === true, resultEvaluation?.personalBest.stern2 === true, resultEvaluation?.personalBest.stern3 === true]; app.innerHTML = `<main class="app-shell result-screen"><section class="result-card" aria-labelledby="result-title"><div class="result-icon" aria-hidden="true">${won ? "✓" : "★"}</div><p class="eyebrow">Einmaleins</p><p class="result-greeting">Gut gespielt, ${escapeHtml(greetingName)}!</p><h1 id="result-title">${headline}</h1><p class="result-copy">${message}</p><div class="result-stars" aria-label="Freigeschaltete Sterne">${earnedStars.map((earned, index) => `<img src="/Stern${index + 1}.png" alt="Stern ${index + 1} ${earned ? "freigeschaltet" : "noch nicht freigeschaltet"}" class="${earned ? "" : "is-muted"}" />`).join("")}</div><div class="result-stats" aria-label="Spielergebnis"><div class="result-stat"><span class="stat-label">Punkte</span><strong>${game.score}</strong></div><div class="result-stat"><span class="stat-label">Aufgaben</span><strong>${game.completedTasks} / ${TOTAL_TASKS}</strong></div><div class="result-stat"><span class="stat-label">Modus</span><strong>${practiceMode === "highscore" ? "Mit Highscore" : "Ohne Highscore"}</strong></div></div><div class="result-highscore" aria-live="polite">${highscoreMessage}</div><button type="button" class="result-button" id="again">Nochmal spielen</button></section></main>`; getRequiredElement<HTMLButtonElement>("#again").addEventListener("click", () => { clearWrongAnswerTimer(); startGame(); }); }
async function submitSchoolHighscore(): Promise<void> { const status = document.querySelector<HTMLElement>("#school-status"); if (!status || !leaderboardClient) return; status.textContent = "Eintrag wird gespeichert …"; const game = controller.getState().game; const record = { studentId: student.studentId, name: student.name, className: student.className, score: game.score, achievedAt: new Date().toISOString(), stern1: resultEvaluation?.personalBest.stern1 ?? game.stern1, stern2: resultEvaluation?.personalBest.stern2 ?? false, stern3: resultEvaluation?.personalBest.stern3 ?? game.stern3 }; try { await leaderboardClient.submit(record); status.textContent = "Erfolgreich eingetragen."; } catch (error) { status.textContent = error instanceof Error ? error.message : "Eintrag konnte nicht gespeichert werden."; } }
function startGame(): void { clearGameTimer(); clearAnswerFeedbackTimer(); clearWrongAnswerTimer(); engine = new GameEngine(); controller = new GameController(engine); controller.start(); screen = "game"; answerPresentation = null; resultEvaluation = null; renderGameScreen(); gameTimer = window.setInterval(() => { controller.tick(); renderGameState(); if (controller.getState().game.phase !== "playing") { clearGameTimer(); const game = controller.getState().game; recordFrameUnlocks(student.studentId, game.errorRate, game.round); resultEvaluation = evaluateResult(student, game.score, { stern1: game.stern1, stern3: game.stern3 }); void leaderboardClient?.submit(resultEvaluation.personalBest, false).catch(() => { /* private synchronization may be retried on the next game */ }); renderResultScreen(); } }, 80); }
function scheduleNextTaskAfterWrongAnswer(): void { clearWrongAnswerTimer(); wrongAnswerTimer = window.setTimeout(() => { wrongAnswerTimer = null; if (screen !== "game") return; controller.advanceAfterWrongAnswer(); renderGameState(); }, WRONG_ANSWER_TOTAL_MS); }
