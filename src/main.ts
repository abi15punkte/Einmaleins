...
function renderResultScreen(): void { clearGameTimer(); clearAnswerFeedbackTimer(); const game = controller.getState().game; const won = game.phase === "won"; const headline = won ? "Geschafft!" : "Die Zeit ist vorbei"; const message = won ? "Du hast alle Aufgaben vor Ablauf der zehn Minuten gelöst." : "Dein Ergebnis steht fest."; const highscoreMessage = resultEvaluation?.isNewPersonalBest ? "🏆 Neuer persönlicher Highscore!" : `Persönlicher Rekord: ${resultEvaluation?.personalBest.score ?? loadPersonalHighscore(student.studentId)?.score ?? 0}`; const greetingName = getGreetingName(student.name); app.innerHTML = `<main class="app-shell result-screen"><section class="result-card" aria-labelledby="result-title"><div class="result-icon" aria-hidden="true">${won ? "✓" : "★"}</div><p class="eyebrow">Einmaleins</p><p class="result-greeting">Gut gespielt, ${escapeHtml(greetingName)}!</p><h1 id="result-title">${headline}</h1><p class="result-copy">${message}</p><div class="result-stats" aria-label="Spielergebnis"><div class="result-stat"><span class="stat-label">Punkte</span><strong>${game.score}</strong></div><div class="result-stat"><span class="stat-label">Aufgaben</span><strong>${game.completedTasks} / ${TOTAL_TASKS}</strong></div><div class="result-stat"><span class="stat-label">Modus</span><strong>${practiceMode === "highscore" ? "Mit Highscore" : "Ohne Highscore"}</strong></div></div><div class="result-highscore" aria-live="polite">${highscoreMessage}</div><button type="button" class="result-button" id="again">Noch eine Runde</button></section></main>`; getRequiredElement<HTMLButtonElement>("#again").addEventListener("click", () => { clearWrongAnswerTimer(); startGame(); }); }
async function submitSchoolHighscore(): Promise<void> { const status = document.querySelector<HTMLElement>("#school-status"); if (!status || !leaderboardClient) return; status.textContent = "Eintrag wird gespeichert …"; const game = controller.getState().game; const record = { studentId: student.studentId, name: student.name, className: student.className, score: game.score, achievedAt: new Date().toISOString(), stern1: false, stern2: false, stern3: false }; try { await leaderboardClient.submit(record); status.textContent = "Erfolgreich eingetragen."; } catch (error) { status.textContent = error instanceof Error ? error.message : "Eintrag konnte nicht gespeichert werden."; } }
function startGame(): void { clearGameTimer(); clearAnswerFeedbackTimer(); clearWrongAnswerTimer(); engine = new GameEngine(); controller = new GameController(engine); controller.start(); screen = "game"; answerPresentation = null; resultEvaluation = null; renderGameScreen(); gameTimer = window.setInterval(() => { controller.tick(); renderGameState(); if (controller.getState().game.phase !== "playing") { clearGameTimer(); const game = controller.getState().game; resultEvaluation = evaluateResult(student, game.score); renderResultScreen(); } }, 80); }
